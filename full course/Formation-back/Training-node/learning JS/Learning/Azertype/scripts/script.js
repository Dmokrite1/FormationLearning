...
/**
 * Cette fonction affiche une proposition, dans la zone "zoneProposition"
 ...
 * @param {string} email : l'email avec qui il veut partager son score
...
function lancerJeu() {
    ...
    // Créez une fonction pour gérer la validation
function valider() {
...
inputEcriture.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
        event.preventDefault() // Pour éviter le rechargement de la page si le bouton est dans un formulaire
        valider()
    }
})
...
btnValiderMot.addEventListener("click", () => {
    ...
    // événement change sur les boutons radios. 
    ...
    for (let index = 0; index < listeBtnRadio.length; index++) {
        listeBtnRadio[index].addEventListener("change", (event) => {
            ...
            if (event.target.value === "1") {
                ...
                // jouer avec la liste des phrases
    ...
    // événement submit sur le formulaire de partage. 
